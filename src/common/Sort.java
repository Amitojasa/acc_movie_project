...
public final class Sort
{
	...
	private static void printTen(Long[] a) {
		for (int i = 0; i < 20; i++)
			System.out.print(a[i] + " ");
		System.out.println();
	}
...
//        Random rand = new Random(System.currentTimeMillis());
		...
		Long [ ] randomLongKeys = new Long [1000000];

		for (int i = 0; i < rightLimit; i++) {
			randomLongKeys[i] = leftLimit + (long) (Math.random() * (rightLimit - leftLimit));
			// https://www.baeldung.com/java-generate-random-long-float-integer-double
		}

		System.out.println("Lenght of Array: " + randomLongKeys.length);
...
//            a[i] = rand.nextInt(NUM_ITEMS * 5);
		...
		Sort.mergeSort(randomLongKeys);
		end = System.nanoTime();
		System.out.println("Merge sort time: " +(end-start));
		checkSort(randomLongKeys);
...
//			a[i] = rand.nextInt(NUM_ITEMS * 5);
		...
		// Sort
		start = System.nanoTime();
		Sort.heapsort(randomLongKeys);
		end = System.nanoTime();
		System.out.println("Heap Sort Tiem: "+(end-start));
		checkSort(randomLongKeys);
...
//            a[i] = rand.nextInt(NUM_ITEMS * 5);
        ...
        for (int i = 1; i <= 10; i++) {
        	int k = i;
        	Sort.quickSelect( randomLongKeys, k); //NUM_ITEMS / 2 );
            System.out.println("Quickselect:");
//            Sort.print(randomLongKeys);
            System.out.println("kth smallest = " + randomLongKeys[k-1]);
